...
export function MCQCard({
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto border-white/10 bg-card/50 backdrop-blur animate-scale-in">
      <CardHeader className="space-y-4 pb-4">
        ...
        {correctChoiceIds.length > 1 && !isAnswered && (
          <div className="flex items-center gap-2 text-sm text-muted-foreground bg-purple-500/5 border border-purple-500/20 rounded-lg p-3 animate-fade-in">
          ...
          {question.choices.map((choice, index) => {
            ...
            return (
              <div
                ...
                className={`group relative flex items-start space-x-4 p-4 rounded-lg border-2 transition-all duration-200 animate-slide-up active:scale-[0.98] ${
                  status === "correct"
              ...
              >
                ...
                {status === "incorrect" && (
                  <XCircle className="w-5 h-5 text-rose-500 flex-shrink-0 animate-scale-in" />
                )}
                {status === "missed" && (
                  <AlertCircle className="w-5 h-5 text-amber-500 flex-shrink-0 animate-scale-in" />
                )}
              </div>
            );
          })}
